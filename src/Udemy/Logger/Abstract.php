 ...
 */
abstract class Udemy_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param Udemy_Client $client  The current Udemy client
   */
  public function __construct(Udemy_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('Udemy_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('Udemy_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('Udemy_Logger_Abstract', 'date_format');
    $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

    $this->allowNewLines = (bool) $client->getClassConfig(
        'Udemy_Logger_Abstract',
        'allow_newlines'
    );
  }
   ...
   * @throws Udemy_Logger_Exception If $level is invalid
   */
